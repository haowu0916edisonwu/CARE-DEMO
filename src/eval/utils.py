...
def eval_fact_checking(outputs,answers):
    ...
    for output,answer in zip(outputs,answers):
        ...
        has_true = has_answer(["supports", "yes", "true"], output)
        has_false = has_answer(["refutes", "no", "false"], output)
                
                # If both true and false are present, mark as incorrect
        if has_true and has_false:
            result = 0
                # Otherwise check if the correct answer is present
        elif has_answer(answer, output):
            result = 1
        else:
            result = 0
...
def get_substring_match_score(outputs, answers):
    ...
    answer_lengths = []
    for output, answer in zip(outputs,answers):
        if has_answer(answer, output): # EM evaluation (This is not an EM evaluation)
