    ...
    if args.checkpoint_path is not None and 'config.yaml' in os.listdir(Path(args.checkpoint_path).parents[1]):
        ...
        for k, v in vars(args).items():
            ...
            if checkpoint_config.get(k) is not None:
                val = checkpoint_config[k]
                final_val = val['value'] if isinstance(val, dict) and 'value' in val else val
                setattr(args, k, final_val)
    else:
        if args.checkpoint_path is not None:
            # 核心修改 2：使用模糊匹配 (in) 识别模型，同时保护你传入的本地 PHYSICAL_BASE 路径
            checkpoint_path_lower = args.checkpoint_path.lower()
            
            # 只有当 model_name_or_path 不是一个存在的本地路径时，才回退到官方 ID
            is_local_path = os.path.exists(args.model_name_or_path)
            
            if 'mistral' in checkpoint_path_lower:
                if not is_local_path:
                    args.model_name_or_path = 'mistralai/Mistral-7B-Instruct-v0.2'
            elif 'llama' in checkpoint_path_lower:
                if not is_local_path:
                    args.model_name_or_path = 'meta-llama/Meta-Llama-3-8B-Instruct'
            elif 'qwen' in checkpoint_path_lower:
                if not is_local_path:
...
if __name__ == "__main__":
    ...
    if args.checkpoint_path is not None:
        ...
        if os.path.exists(os.path.join(args.checkpoint_path, "pytorch_model.bin.index.json")):
            ...
            # 1. 明确拼接本地权重路径
            target_ckpt = os.path.join(args.checkpoint_path, "ckpt.pth")
            
            # 2. 物理检查：只有文件真的在那里才加载 (借鉴 Llama 成功经验)
            if os.path.exists(target_ckpt):
                print(f"> [SUCCESS] 成功定位本地权重文件: {target_ckpt}")
                ckpt_path = target_ckpt
            else:
                # 3. 如果找不到，打印详细提示并强制退出，防止程序在后台静默挂掉
                print(f"> [FATAL ERROR] 无法在路径 {args.checkpoint_path} 中找到 ckpt.pth")
                print("> 请确认：1. 训练已完成？ 2. 权重已拷贝到 mistral/last 目录？")
                import sys
                sys.exit(1)
